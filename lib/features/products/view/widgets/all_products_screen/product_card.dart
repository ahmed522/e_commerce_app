...
class ProductCard extends StatelessWidget {
  const ProductCard({super.key, required this.product, this.isFavPage = false});
                ...
                children: [
                  ...
                  isFavPage
                      ? BlocBuilder<FavouritesCubit, FavouritesStates>(
                          ...
                          return IconButton(
                            onPressed: (state is FavouritesLoadingState)
                                ? null
                                : () {
                                    final cubit = FavouritesCubit.get(context);
                                    cubit.removeFromFavourites(product.sId!);
                                  },
                      ...
                      : BlocBuilder<AllProductsCubit, AllProductsStates>(
                          ...
                          return ElevatedButton(
                            onPressed: (state is LoadingAddingToCartState)
                                ? null
                                : () {
                                    final cubit = AllProductsCubit.get(context);
                                    cubit.addToCart(context, product.sId!);
                                  },
                            style: ElevatedButton.styleFrom(
                              shape: RoundedRectangleBorder(
                                borderRadius: BorderRadius.circular(8),
